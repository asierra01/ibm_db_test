"""test Store Proc by using python cextension spclient_python
OUT_LANGUAGE
ALL_DATA_TYPES
ONE_RESULT_SET
...
class SpClientPython(CommonTestCase):
    ...
    def __init__(self, testname, extraarg=None):
        super(SpClientPython, self).__init__(testname, extraarg)
    ...
    def test_arr_spclient_python_phones_store_proc(self):
        ...
        try:
            ...
            ret_sp_call = spclient_python.python_extract_array_into_python_ibm_db(self.conn,
                                                                            mylog.info,
                                                                            self.mDb2_Cli.describe_parameters)
            mylog.info("ret_sp_call '%s'" % ret_sp_call)
    ...
    def test_register_store_proc(self):
        ...
        sql_str= """
CREATE TYPE 
   PHONEARRAY AS VARCHAR(20) ARRAY[10000]
@
...
""")
        ...
        try:
            ...
            stmt1, out_array, out_records_in_array = ibm_db.callproc(self.conn, "EXTRACT_ARRAY", (out_array,out_records_in_array,))
            self.mDb2_Cli.describe_parameters(stmt1)
    ...
    def test_spclient_python_array_extract(self):
        ...
        try:
            ...
            if self.EXTRACT_ARRAY_registered :
                hello = spclient_python.python_extract_array_ibm_db(self.conn,
                                                                mylog.info,
                                                                self.mDb2_Cli.describe_parameters,
                                                                self.mDb2_Cli.describe_columns)
            else:
                self.result.addSkip(self, """
we cant run spclient_python.python_extract_array_ibm_db as SP EXTRACT_ARRAY was not registered""")
                return 0
            mylog.info("hello %s" % hello)
    ...
    def test_register_store_proc_array_extract(self):
        ...
        sql_str = """
CREATE TYPE DOUBLEARRAY AS DOUBLE ARRAY[10000]@
...
""" 
    ...
    def drop_table_TESTING_LOAD_FROM_TABLE_FUNCTION_CSV(self):
        sql_str = "DROP TABLE {schema}.TESTING_LOAD_FROM_TABLE_FUNCTION_CSV".format(schema=self.getDB2_USER())
    ...
    def test_array_extract_store_proc_1(self):
        ...
        try:
            ...
            stmt1 = ibm_db.prepare(self.conn, "CALL EXTRACT_ARRAY (?,?)", {ibm_db.SQL_ATTR_CURSOR_TYPE: ibm_db.SQL_CURSOR_KEYSET_DRIVEN})
            self.mDb2_Cli.describe_parameters(stmt1)
...
""".format(user=self.getDB2_USER())
        ...
        try:
            ...
            mylog.info("store proc executed")
            self.mDb2_Cli.describe_parameters(stmt)
