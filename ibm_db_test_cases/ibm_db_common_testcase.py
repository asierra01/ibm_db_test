...
class CommonTestCase(unittest.TestCase):
    ...
    def __init__(self, test_name, extra_arg=None, verbosity=False):
        """
        Parameters
        ----------
        test_name   : :obj:`str`
        extra_arg   : :obj:`int` = 42 just to prove I can pass extra arg to a test case
        verbosity   : :obj:`bool`
        """
        ...
        mylog.info("initialized '%s' " % test_name)
    ...
    def __del__(self):
        if mylog.level == logging.DEBUG:
            print("CommonTestCase.__del__ '%s'" % self.test_name)
    ...
    def run_statement(self, sql_str):
        ...
        for sql_1 in sql_str.split("@"):
            ...
            try:
                ...
                if "SQL0601N" in str(i): #The name of the object to be created is identical to the existing name 
                    mylog.warning("executing %s\nerror\n%s\n" % (sql_1, str(i)))
                    continue
...
%s""" % (cls.__name__,
    ...
    def if_bufferpool_present(self, bpname):
        sql_str = """
SELECT 
    BPNAME
FROM 
...
""" % bpname.upper()
        ...
        while dictionary:
            if dictionary['BPNAME'].upper() == bpname.upper():
                mylog.info("Bufferpool '%s' found" % bpname.upper())
                found = True
                break
            dictionary = ibm_db.fetch_both(stmt_select)
    ...
    def if_tablespace_present(self, tbsp_name):
        sql_str = """
SELECT
    TBSP_NAME
FROM
...
""" % (tbsp_name.upper())
        ...
        while dictionary :
            ...
            if dictionary ['TBSP_NAME'].upper() == tbsp_name.upper():
                found = True
                mylog.debug("Tablespace found '%s'" % tbsp_name.upper())
                break
            dictionary = ibm_db.fetch_both(stmt1)
        ibm_db.free_result(stmt1)
        return found
...
""" % routinename
            ...
            while dictionary:
                ...
                mylog.debug("SPECIFICNAME '%s' ROUTINENAME '%s'='%s'" % (specific_name, routine_name, routinename.upper()))
    ...
    def if_table_present(self, conn, table_name, schema_name):
        ...
        try:
            ...
            tbls_stament = ibm_db.tables(conn, None, pattern, table_name.upper(), None)
...
stmt_errormsg : '%s'""" %  (ex, type(ex), stmt_error, stmt_errormsg))
    ...
    def set_table_keys(self, type):
        if type:
            ...
            table_keys.set_deco(Texttable.HEADER)
            table_keys.header(str_header.split())
            table_keys.set_cols_width( [30, 90])
            table_keys.set_header_align(['l', 'l'])
            
        return table_keys


    def print_keys(self, one_dictionary, human_format=False, Print=True, print_0=True, table_name='', display_type=False):
        ...
        """
        if not isinstance(one_dictionary, dict):
            mylog.error("one_dictionary is not type dict")
            mylog.info("who called '%s' " % (sys._getframe().f_back.f_code.co_name))
            return
        table_keys = self.set_table_keys(display_type)
        ...
        for key in one_dictionary.keys():
            ...
            if human_format:
                #mylog.info("key %s %s" % (key, type(one_dictionary[key])))
...
DRIVER_ODBC_VER       : '%s'
DATA_SOURCE_NAME      : '%s'
ODBC_SQL_CONFORMANCE  : '%s'
APPL_CODEPAGE         : %04s
CONN_CODEPAGE         : %04s
ibm_db.__file__       : '%s'
ibm_db_dbi.__ver__    : '%s'
ibm_db_dbi.__file__   : '%s'
""" % (self.server_info.DBMS_NAME,
       self.server_info.DBMS_VER,
       self.server_info.DB_NAME,
       self.client_info.DRIVER_NAME,
       self.client_info.DRIVER_VER,
       self.client_info.ODBC_VER,
       self.client_info.DRIVER_ODBC_VER,
       self.client_info.DATA_SOURCE_NAME,
       self.client_info.ODBC_SQL_CONFORMANCE,
       ...
       ibm_db_dbi.__file__))
            try:
                ...
                mylog.info("os.getpid %d" % os.getpid())

                mytable = self.my_table_dll_so()

                if hasattr(p, "memory_maps"):
                    for dll in p.memory_maps():
                        row = ["so/dll", "%s" % dll.path]
                        mytable.add_row(row)
                mylog.debug("dlls\n%s\n" % mytable.draw())
